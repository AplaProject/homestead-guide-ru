...
Также существуют специальные атрибуты.
**copyclipboard** будет преобразовано в *onClick="CopyToClipboard('#clipboard')"*
...
Существует переменная **#lang#**, которая равна первому языку, указанному в *Accept-Language* в заголовке запроса.
...
Now([format],[interval]) 
==============================
Функция выводит текущее время в указанном формате, по умолчанию выводится  в UNIX-формате (число секунд с 1970 года), если в качестве формата указано *datetime*, то дата и время выводится в виде YYYY-MM-DD HH:MI:SS. Во втором параметре можно указать интервал, например, *+5 days*.
...
GetMap(name, table, idval, column) 
==============================
Функция получает значение в колонке *column* из таблицы *table* c **id** равным *idval*. Значение должно иметь такой вид [key1, value1],[key2,value2],... Далее она распределяет эти данные в список с двумя колонками **key** и **value**. После этого вы можете обращаться к этим колонкам в функциях **ListVal** или **ForList**.

* *name* - имя, по которому извлекается конкретная запись из полученного списка с помощью функций **ListVal** или **ForList**;
* *table* - имя таблицы для получения значения;
* *idval* - значение поля **id**; 
* *column* - имя колонки, из которой берется значение.
...
Map(coords, [class]) 
==============================
Функция выводит на страницу контейнер google maps с координатами указанными в параметре *coords* в формате: {"center_point":["23.907173","54.333531"], "zoom":"7", "cords":[["23.915970","54.239502"],["23.654588","55.371094"],["22.958393","55.316162"]]}. Высоты контейнера берется из значения предварительно определенной переменной #hmap# (по умолчанию 100 пикселей), ширина растягивается на максимально возможную величину. Вторым параметром можно указывать класс для div, а также дополнительные параметры для карты. Например, вы можете в параметре **maptype** указать один из следующих типов карты: **roadmap, satellite, hybrid, terrain**. Например, *maptype=satellite*. По умолчанию, тип карты *terrain*. Также можно указывать высоту отличную от *hmap* по умолчанию. Для этого укажите атрибут hmap во втором параметре, например, **hmap=500**.

.. code:: js

   Map(`{"center_point":["23.907173","54.333531"], "zoom":"7", "cords":[["23.915970","54.239502"],["23.654588","55.371094"],["22.958393","55.316162"]]}`, maptype=satellite hmap=250 )

MapPoint(coords, [class]) 
==============================
Функция выводит на страницу контейнер google maps с маркером по координатам указанным в параметре *cords*. Высоты контейнера берется из значения предварительно определенной переменной #hmap# (по умолчанию 100 пикселей), ширина растягивается на максимально возможную величину. Вторым параметром можно указывать класс для div, а также дополнительные параметры для карты. Например, вы можете в параметре **maptype** указать один из следующих типов карты: **roadmap, satellite, hybrid, terrain**. Например, *maptype=satellite*. По умолчанию, тип карты *terrain*. Также можно указывать высоту отличную от *hmap* по умолчанию. Для этого укажите атрибут hmap во втором параметре, например, **hmap=500**.

.. code:: js

   MapPoint(`{"center_point":["23.907173","54.333531"], "zoom":"7", "cords":["23.915970","54.239502"]}`, maptype=hybrid myclass)
